...
void sendmsg (char *user, char *target, char *msg) {
	...
	struct message messageLine;
	int server;
	signal(SIGPIPE, SIG_IGN); //sigpipe and sig ignore are used as signal flags
	strcpy(messageLine.source, user); 	//copies the message to be sent to user, target, and msg
	strcpy(messageLine.target, target);
	strcpy(messageLine.msg, msg);
	server = open("serverFIFO", O_WRONLY); //opens server for the serverFIFO and write only
	write(server, &messageLine, sizeof(struct message)); //writes to the message
	close(server);
}
